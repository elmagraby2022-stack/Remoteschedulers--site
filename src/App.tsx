...
const FORMSPREE_URL = 'https://formspree.io/f/mdavgvaj';
...
const Header = ({ 
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 transition-all duration-500 ${isMobileMenuOpen ? 'z-[9999]' : 'z-[50]'}`}>
      ...
      <nav className={`transition-all duration-500 border-b ${isScrolled ? 'glass-nav py-3 shadow-2xl shadow-navy/5 border-navy/5' : 'bg-white-off/95 md:bg-transparent py-6 border-transparent'}`}>
        <motion.div className="absolute top-0 left-0 right-0 h-[3px] bg-gold origin-left" style={{ scaleX }} />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-10">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-[11px] font-bold tracking-[0.2em] uppercase text-navy/70 hover:text-gold transition-all relative group">
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-px bg-gold transition-all group-hover:w-full"></span>
              </a>
            ))}
            <button onClick={onOpenQuote} className="bg-navy text-gold hover:bg-gold hover:text-navy px-8 py-3 text-[11px] font-bold tracking-widest uppercase transition-all transform hover:-translate-y-1 shadow-lg shadow-navy/10 active:scale-95">
              Get a Quote
            </button>
          </div>

          <button className="md:hidden text-navy p-2 hover:bg-gold/10 rounded-full transition-colors" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 backdrop-blur-md z-[10000] md:hidden bg-navy/85 overscroll-contain" />
              <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="fixed inset-0 w-screen h-screen bg-navy z-[10001] md:hidden shadow-4xl overflow-y-auto overscroll-contain flex flex-col">
                <div className="w-full flex-1 flex flex-col justify-between p-6 xs:p-8 min-h-full">
                  <div className="flex justify-between items-center mb-10 shrink-0">
                    <div className="flex flex-col leading-none">
                      <span className="font-condensed text-xl font-extrabold tracking-wider text-gold uppercase">Remote <span className="text-white-off">Schedulers</span></span>
                    </div>
                    <button className="text-gold p-2 hover:bg-gold/10 rounded-full transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                      <X size={28} />
                    </button>
                  </div>
                  <div className="flex flex-col space-y-6 xs:space-y-8 my-auto py-6 items-center text-center">
                    {navLinks.map((link, i) => (
                      <motion.a initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.05 }} key={link.name} href={link.href}
                  ...
                  <div className="mt-auto pt-6 border-t border-white-off/10 space-y-6 shrink-0 w-full max-w-sm mx-auto">
                    <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                      onClick={() => { document.body.style.overflow = ''; setIsMobileMenuOpen(false); onOpenQuote(); }}
  ...
  return (
    <section className="relative min-h-[90vh] md:min-h-screen flex items-center md:items-end pt-40 md:pt-24 overflow-hidden bg-navy">
      <div className="absolute inset-0 z-0">
        <motion.img initial={{ scale: 1.1, opacity: 0 }} animate={{ scale: 1, opacity: 0.35 }} transition={{ duration: 1.5, ease: "easeOut" }}
          src="https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=1400&q=80" alt="Construction Site" className="w-full h-full object-cover" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pb-12 md:pb-24 w-full">
        <div className="max-w-5xl">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }}
            ...
            Expert Primavera P6 and CPM scheduling services for contractors. We build clear, professional schedules that help you finish your project on time and on budget.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }} className="flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-5">
            <button onClick={onOpenQuote} className="group relative bg-gold hover:bg-gold-light text-navy px-6 xs:px-10 sm:px-12 py-4 sm:py-5 text-xs font-bold tracking-[0.2em] uppercase flex items-center justify-center sm:justify-start gap-3 overflow-hidden shadow-2xl shadow-gold/20 active:scale-95 transition-all">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2, duration: 2 }} className="absolute bottom-24 right-10 hidden xl:flex flex-col gap-10 opacity-30 pointer-events-none">
        <div className="flex items-center gap-3 text-gold"><ShieldCheck size={32} /><div className="h-px w-20 bg-gold/50"></div></div>
        <div className="flex items-center gap-3 text-gold"><TrendingUp size={32} /><div className="h-px w-20 bg-gold/50"></div></div>
        <div className="flex items-center gap-3 text-gold"><Globe size={32} /><div className="h-px w-20 bg-gold/50"></div></div>
      </motion.div>
      <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 lg:hidden">
...
const Stats = () => {
  ...
  return (
    <div className="bg-navy border-t border-gold/20 grid grid-cols-2 lg:grid-cols-4 relative z-20 overflow-hidden">
      {stats.map((stat, idx) => (
        <div key={stat.label} className={`py-8 md:py-12 px-4 sm:px-8 text-center border-gold/10 ${idx !== stats.length - 1 ? 'lg:border-r' : ''} ${idx % 2 === 0 ? 'border-r lg:border-r' : ''} border-b lg:border-b-0`}>
          <motion.div initial={{ opacity: 0, scale: 0.5 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="font-display text-4xl sm:text-5xl md:text-6xl text-gold mb-2">{stat.value}</motion.div>
    ...
    <section id="services" className="py-20 sm:py-24 px-4 sm:px-6 bg-white-off overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 sm:mb-20 gap-8">
          <div>
            <motion.span initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-[10px] font-bold tracking-[0.4em] text-gold uppercase mb-4 block">Excellence in Planning</motion.span>
          ...
          <p className="text-gray-soft max-w-sm text-base leading-relaxed border-l-2 border-gold pl-6 py-1">From baseline schedules to critical delay claims — we provide the technical logic required for elite construction performance.</p>
        </div>
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} variants={{ hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.1 } } }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-gold/15 border border-gold/15 group">
          {services.map((s) => (
            <motion.div key={s.id} variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="bg-white-off p-8 sm:p-12 relative group/card card-hover overflow-hidden">
              ...
              <div className="text-gold/5 font-display text-4xl sm:text-5xl absolute top-6 sm:top-8 right-6 sm:right-8 transition-all group-hover/card:text-gold/10 group-hover/card:scale-110">{s.id}</div>
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-navy text-gold flex items-center justify-center mb-8 sm:mb-10 group-hover/card:bg-gold group-hover/card:text-navy transition-colors duration-500 rounded-sm shadow-xl shadow-navy/5">{s.icon}</div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 sm:mb-24 gap-10">
          <div className="max-w-2xl">
            <motion.span initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-[10px] font-bold tracking-[0.4em] text-gold uppercase mb-5 block">Efficiency Guaranteed</motion.span>
          ...
          <p className="text-white-off/50 max-w-sm text-base sm:text-lg leading-relaxed font-light border-l border-gold/30 pl-6 sm:pl-8">We've removed the friction from construction planning. Experience a digital-first approach that prioritizes speed and reliability.</p>
        </div>
        <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-x-8 gap-y-16 sm:gap-12">
          ...
          {steps.map((step, i) => (
            <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="text-center group">
              <div className="w-14 h-14 rounded-full border-2 border-gold/50 flex items-center justify-center bg-navy text-gold font-condensed font-extrabold text-lg mx-auto mb-10 relative z-10 shadow-[0_0_20px_rgba(201,168,76,0.1)] group-hover:scale-110 group-hover:border-gold transition-all duration-500 ease-out">{step.num}</div>
    ...
    <section id="why" className="grid grid-cols-1 lg:grid-cols-2 min-h-[70vh] lg:min-h-[80vh] overflow-hidden">
      <div className="bg-navy p-8 sm:p-12 lg:p-24 flex flex-col justify-center relative overflow-hidden border-b lg:border-b-0 border-gold/10">
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="font-condensed text-3xl xs:text-4xl sm:text-6xl md:text-8xl font-extrabold text-white-off mb-6 sm:mb-12 uppercase leading-[0.9] sm:leading-[0.85]">Project Controls<br /><span className="text-gold">Redefined</span></h2>
          <div className="space-y-6 sm:space-y-8">
            {['Experienced scheduling experts with real construction field knowledge.','Fast delivery: Project schedules completed in 5-7 business days.','Clear, project-based pricing with no hidden costs.','Experts in Primavera P6, project logic, and delay analysis.','Professional scheduling team with clear and fast communication.'].map((point, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ x: 10, backgroundColor: "rgba(201, 168, 76, 0.05)" }} className="flex gap-4 sm:gap-6 items-start group/why p-3 sm:p-4 -m-3 sm:-m-4 rounded-sm cursor-default transition-all duration-300">
                <div className="mt-1 w-6 h-6 sm:w-7 sm:h-7 bg-gold/10 border border-gold/30 flex items-center justify-center text-gold shrink-0 group-hover/why:bg-gold group-hover/why:text-navy transition-all duration-300"><CheckCircle2 size={14} /></div>
      ...
      <div className="h-[400px] sm:h-[500px] lg:h-auto overflow-hidden relative group">
        <img src="https://images.unsplash.com/photo-1589939705384-5185137a7f0f?w=1000&q=80" alt="Site Review" className="w-full h-full object-cover grayscale transition-all duration-1000 group-hover:scale-110" />
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="absolute bottom-6 sm:bottom-12 left-6 sm:left-12 right-6 sm:right-12 md:right-auto md:w-80 bg-white-off p-6 sm:p-10 shadow-2xl border-l-[6px] border-gold backdrop-blur-sm bg-white-off/95">
    ...
    <section id="testimonials" className="py-24 sm:py-32 px-4 sm:px-6 bg-white-off relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 sm:mb-24">
          <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-[10px] font-bold tracking-[0.4em] text-gold uppercase mb-5 block">Verified Performance</motion.span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10">
          {reviews.map((r, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="bg-[#EDEAE2] p-8 sm:p-12 border-t-[5px] border-gold shadow-2xl hover:shadow-gold/10 transition-all duration-500 relative group overflow-hidden">
              ...
              <div className="flex items-center gap-4 sm:gap-5">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-sm bg-navy text-gold flex items-center justify-center font-extrabold text-xs sm:text-sm shadow-lg">{r.name.charAt(0)}</div>
                <div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(FORMSPREE_URL, {
        ...
        body: JSON.stringify({ email, _subject: 'Free Audit Request - Remote Schedulers' }),
      });
      setIsSubmitted(true);
      setEmail('');
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          ...
          <form onSubmit={handleSubmit} className="max-w-2xl mx-auto flex flex-col md:flex-row gap-0 group shadow-3xl bg-white-off/5 border border-gold/20">
            <input type="email" placeholder="Enter your professional email..." value={email} onChange={(e) => setEmail(e.target.value)}
              className="flex-1 px-6 sm:px-8 py-5 sm:py-6 text-white-off focus:outline-none focus:bg-white-off/10 transition-all text-sm sm:text-base font-light placeholder:text-white-off/20 bg-transparent" required />
            <button type="submit" disabled={isSubmitting} className="relative bg-gold hover:bg-gold-light text-navy px-10 sm:px-12 py-5 sm:py-6 font-extrabold tracking-[0.2em] text-[10px] sm:text-[11px] uppercase transition-all overflow-hidden group/btn active:scale-[0.98] disabled:opacity-70">
              <span className="relative z-10">{isSubmitting ? '...' : 'Get Free Audit'}</span>
          ...
          <AnimatePresence>
            {isSubmitted && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
                className="mt-8 sm:mt-10 text-gold font-bold text-sm sm:text-base flex items-center justify-center gap-3 bg-gold/10 py-3 sm:py-4 px-6 sm:px-8 border border-gold/20 backdrop-blur-sm mx-auto w-fit">
    ...
    <footer className="bg-[#050C14] py-16 sm:py-24 px-4 sm:px-6 border-t border-gold/10 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-16 gap-x-12 sm:gap-16 mb-16 sm:mb-20 pb-16 sm:pb-20 border-b border-white-off/5">
          <div className="col-span-1 sm:col-span-2">
            <div className="logo mb-6 sm:mb-8">
              <span className="font-condensed text-xl sm:text-2xl font-extrabold tracking-widest text-white-off uppercase">Remote <span className="text-gold">Schedulers</span></span>
            </div>
            <p className="text-white-off/30 text-sm sm:text-base leading-relaxed max-w-md font-light">Engineering professional CPM schedules for global contractors since 2018. Our mission is to translate complex construction logic into clear, defensive, and high-performance plans.</p>
            <div className="flex gap-4 sm:gap-6 mt-8 sm:mt-10">
              <div className="w-10 h-10 border border-gold/20 flex items-center justify-center text-gold/40 hover:text-gold hover:border-gold transition-all duration-300 cursor-pointer"><Globe size={18} /></div>
              <div className="w-10 h-10 border border-gold/20 flex items-center justify-center text-gold/40 hover:text-gold hover:border-gold transition-all duration-300 cursor-pointer"><ShieldCheck size={18} /></div>
            </div>
          </div>
          <div className="space-y-6 pt-8 border-t border-white-off/5 sm:pt-0 sm:border-0">
            ...
            <ul className="space-y-4 text-white-off/40 text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em]">
              {['Services', 'Process', 'Why Us', 'Contact'].map(link => (
                <li key={link}><a href={`#${link.toLowerCase().replace(' ', '')}`} className="hover:text-gold transition-colors inline-block hover:translate-x-1 duration-300">{link}</a></li>
              ))}
            </ul>
          </div>
          <div className="space-y-6 pt-8 border-t border-white-off/5 sm:pt-0 sm:border-0">
            ...
            <div className="space-y-6 text-white-off/40 text-[11px] sm:text-xs font-bold uppercase tracking-[0.15em]">
              <a href="mailto:info@remoteschedulers.com" className="flex items-center gap-3 hover:text-gold transition-colors duration-300 break-all"><Mail size={14} className="text-gold/50 shrink-0" /> info@remoteschedulers.com</a>
              <a href="tel:7327162718" className="flex items-center gap-3 hover:text-gold transition-colors duration-300 break-all"><Phone size={14} className="text-gold/50 shrink-0" /> 732-716-2718</a>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 sm:gap-8 text-center md:text-left">
          <p className="text-white-off/20 text-[9px] sm:text-[10px] font-bold tracking-[0.2em] uppercase">&copy; {currentYear} Remote Schedulers Planning Group. All Digital Assets Protected.</p>
    ...
    <AnimatePresence>
      {isVisible && !isHidden && (
        <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
          ...
          className="fixed bottom-8 right-8 z-[100] w-14 h-14 bg-gold text-navy rounded-sm shadow-2xl flex items-center justify-center transition-transform hover:scale-110 active:scale-95 group overflow-hidden border border-navy/5">
...
const QuoteModal = ({ isOpen, onClose, prefilledData }: { isOpen: boolean; onClose: () => void; prefilledData?: { projectType?: string; challenges?: string; projectSize?: string } | null; }) => {
  const [formData, setFormData] = useState({ name: '', company: '', email: '', projectType: 'Commercial', projectSize: '', challenges: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({ name: '', company: '', email: '', projectType: prefilledData?.projectType || 'Commercial', projectSize: prefilledData?.projectSize || '', challenges: prefilledData?.challenges || '' });
      setIsSuccess(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(FORMSPREE_URL, {
        ...
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({
          _subject: `New Quote Request: ${formData.projectType} Project - ${formData.company}`,
          name: formData.name,
      ...
      setIsSuccess(true);
      setTimeout(() => { onClose(); setIsSuccess(false); }, 3000);
    } catch {
      // silent fail
    } finally {
      setIsSubmitting(false);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[30001] overflow-y-auto bg-navy/80 backdrop-blur-xl flex items-start sm:items-center justify-center p-4 md:p-10">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
            ...
            <button onClick={onClose} aria-label="Close modal" className="absolute top-4 right-4 sm:top-6 sm:right-6 w-10 h-10 rounded-full bg-navy/5 text-navy/60 hover:text-navy hover:bg-navy/10 transition-all flex items-center justify-center cursor-pointer z-20">
              <X size={20} />
            </button>
            {isSuccess ? (
              <div className="py-20 text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-20 h-20 bg-gold/10 text-gold rounded-full flex items-center justify-center mx-auto mb-8">
                  <CheckCircle2 size={40} />
                </motion.div>
                <h3 className="font-condensed text-4xl font-extrabold text-navy uppercase mb-4">Request Sent!</h3>
                <p className="text-gray-soft text-lg">Your quote request has been received. Our team will respond within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                  <div className="space-y-1">
                    ...
                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-navy/40">Company Name</label>
                    <input required type="text" value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm" />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-navy/40">Professional Email</label>
                    <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-navy/40">Project Type</label>
                    <select value={formData.projectType} onChange={e => setFormData({...formData, projectType: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm appearance-none">
                      <option>Commercial</option><option>Infrastructure</option><option>Industrial</option><option>Residential</option><option>Other</option>
                    </select>
                  </div>
                </div>
                <div className="space-y-1 mb-5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy/40">Project Size / Budget</label>
                  <input type="text" placeholder="e.g. $10M / 50,000 SF" value={formData.projectSize} onChange={e => setFormData({...formData, projectSize: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm" />
                </div>
                <div className="space-y-1 mb-8">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy/40">Key Challenges / Requirements</label>
                  <textarea rows={2} value={formData.challenges} onChange={e => setFormData({...formData, challenges: e.target.value})} className="w-full bg-navy/5 border-b border-navy/10 px-3 py-2.5 focus:outline-none focus:border-gold transition-colors font-medium text-navy text-sm resize-none" placeholder="Briefly describe your scheduling needs..." />
                </div>
                <div className="flex flex-col-reverse sm:flex-row gap-3">
                  <button type="button" onClick={onClose} className="flex-1 bg-[#ECE9DF] hover:bg-[#E2DDD0] text-navy py-4 font-bold uppercase tracking-[0.2ex] text-[10px] transition-all text-center cursor-pointer">Cancel & Close</button>
                  <button type="submit" disabled={isSubmitting} className="flex-[2] bg-navy text-gold py-4 px-6 font-bold uppercase tracking-[0.2em] text-[10px] sm:text-xs hover:bg-gold hover:text-navy transition-all flex items-center justify-center gap-2.5 disabled:opacity-50 cursor-pointer">
                    {isSubmitting ? <div className="w-5 h-5 border-2 border-gold border-t-transparent rounded-full animate-spin"></div> : <><span>Generate Professional Brief</span><ChevronRight size={16} /></>}
                  </button>
                </div>
              </form>
            )}
...
const P6Estimator = ({ onOpenQuoteWithDetails }: { onOpenQuoteWithDetails: (type: string, budget: string, speed: string) => void }) => {
  ...
  const calculatePrice = () => {
    ...
    return { original: Math.round(basePrice), discounted: Math.round(basePrice * 0.7) };
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {types.map(t => <button key={t} onClick={() => setProjectType(t)} className={`py-3 px-3 text-[11px] font-bold uppercase border transition-all truncate ${projectType === t ? 'border-gold bg-gold text-navy' : 'border-white-off/10 hover:border-gold/50 text-white-off/70 bg-navy/20'}`}>{t}</button>)}
          </div>
        </div>
        <div className="space-y-3">
          <label className="text-[10px] font-bold uppercase tracking-widest text-gold/80 block">Estimated Budget / Size</label>
          <div className="grid grid-cols-2 gap-2">
            {budgets.map(b => <button key={b} onClick={() => setProjectBudget(b)} className={`py-3 px-3 text-[11px] font-bold uppercase border transition-all truncate ${projectBudget === b ? 'border-gold bg-gold text-navy' : 'border-white-off/10 hover:border-gold/50 text-white-off/70 bg-navy/20'}`}>{b}</button>)}
          </div>
        </div>
        <div className="space-y-3">
          <label className="text-[10px] font-bold uppercase tracking-widest text-gold/80 block">Turnaround Option</label>
          <div className="flex flex-col gap-2">
            {speeds.map(s => <button key={s} onClick={() => setSpeed(s)} className={`py-3 px-4 text-[11px] font-bold uppercase border transition-all text-left flex justify-between items-center ${speed === s ? 'border-gold bg-gold text-navy' : 'border-white-off/10 hover:border-gold/50 text-white-off/70 bg-navy/20'}`}>
              <span>{s}</span>
      ...
      <div className="border-t border-white-off/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-8 bg-black/30 p-6 sm:p-8 rounded-sm">
        {projectBudget === 'Higher than $20M' ? (
          <div className="text-center md:text-left flex-1">
            ...
            <div className="text-xl sm:text-2xl font-condensed font-extrabold text-white-off uppercase leading-tight mt-2">Please reach out by email or phone to discuss further</div>
            <p className="text-[11px] text-white-off/50 mt-3 font-sans">For projects exceeding $20M, custom baseline constraints apply. Call <a href="tel:7327162718" className="text-gold font-bold hover:underline">732-716-2718</a> or email <a href="mailto:info@remoteschedulers.com" className="text-gold font-bold hover:underline">info@remoteschedulers.com</a>.</p>
          </div>
        ) : (
        ...
        <button onClick={() => onOpenQuoteWithDetails(projectType, projectBudget, speed)} className="w-full md:w-auto bg-gold hover:bg-gold-light text-navy py-5 px-10 text-[11px] font-extrabold uppercase tracking-widest transition-transform transform active:scale-95 duration-300 shadow-xl shadow-gold/10 shrink-0">
...
export default function App() {
  ...
  const handleOpenQuoteWithDetails = (type: string, budget: string, speed: string) => {
    setPrefilledData({ projectType: type, projectSize: budget, challenges: `Priority setup: ${speed}. Please provide a 30% discounted estimate of professional scheduling services for this project.` });
  ...
  return (
    <div className="min-h-screen bg-white-off overflow-x-hidden selection:bg-gold selection:text-navy">
      ...
      <Header isMobileMenuOpen={isMobileMenuOpen} setIsMobileMenuOpen={setIsMobileMenuOpen} onOpenQuote={() => setIsQuoteModalOpen(true)} />
      ...
      <ContactCTA />
      <Footer />
      <QuoteModal isOpen={isQuoteModalOpen} onClose={() => { setIsQuoteModalOpen(false); setPrefilledData(null); }} prefilledData={prefilledData} />
    </div>
  );
}
